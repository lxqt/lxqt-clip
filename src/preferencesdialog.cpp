    ...
    QDialog(parent)
{
    ...
    QAction *actUseDefault = new QAction(QIcon(Preferences::DEFAULT_ICON_PATH), tr("Use default icon"), this);
    ...
    connect(actLoadFromFile, &QAction::triggered, this, &PreferencesDialog::selectIconFromFile);
    connect(actUseDefault, &QAction::triggered, this, &PreferencesDialog::useDefaultIcon);
...
void PreferencesDialog::accept()
{
    ...
    {
        Item item(listWidget->item(i)->text());
        list.append(item);
    }
    Preferences::Instance()->saveStickyItems(list);

    Preferences::Instance()->savePathToIcon(getNewTrayIcon());

    QDialog::accept();
}

void PreferencesDialog::selectIconFromFile()
{
...
void PreferencesDialog::useDefaultIcon()
{
  temporarilyRembemberNewTrayIcon(Preferences::DEFAULT_ICON_PATH);
}
...
void PreferencesDialog::stickyRemoveButton_clicked()
{
...
void PreferencesDialog::stickyUpButton_clicked()
{
...
void PreferencesDialog::stickyDownButton_clicked()
{
    int row = listWidget->currentRow();
...
void PreferencesDialog::listWidget_currentRowChanged(int row)
{
...
void PreferencesDialog::temporarilyRembemberNewTrayIcon(const QString &path) {
  buttonIconImage->setIcon(QIcon(path));
  buttonIconImage->setProperty("tray_icon", path);
}

QString PreferencesDialog::getNewTrayIcon() const
{
